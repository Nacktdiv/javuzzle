...
export const saveToSyncQueue = (action: string, payload: any) => {
  const db = getDb()
  db.runSync(
...
export const LAST_STUDY_DATE_KEY = "@last_study_date";

/**
 * Memeriksa & mereset hari di SQLite Lokal + Sync Queue secara atomic
 */
export async function executeDailyResetIfNeeded(userId: string): Promise<boolean> {
  if (!userId) return false;

  const todayStr = new Date().toISOString().split("T")[0];
  const lastDate = await AsyncStorage.getItem(LAST_STUDY_DATE_KEY);
  console.log('data lastdate: ', lastDate)

  if (lastDate !== todayStr) {
    console.log("🌅 [DAILY RESET] Hari baru dideteksi! Menjalankan reset ke 0...");

    const db = getDb();
    if (!db) return false;

    try {
      // 1. Ambil user saat ini dari SQLite
      const currentUser = db.getFirstSync("SELECT * FROM users WHERE id = ?;", [userId]) as userType | null;
      if (!currentUser) return false;

      // 2. Hapus antrean sync UPDATE_STUDY_TIME lama agar payload menit tua tidak terkirim
      db.runSync(`DELETE FROM sync_queue WHERE action = 'UPDATE_STUDY_TIME';`);

      // 3. Update SQLite lokal (today_minutes = 0)
      db.runSync(
        `UPDATE users SET today_minutes = 0, is_synced = 0 WHERE id = ?;`,
        [userId]
      );

      // 4. Masukkan antrean reset 0 menit ke sync_queue
      db.runSync(
        `INSERT INTO sync_queue (action, payload, created_at) VALUES (?, ?, ?);`,
        [
          "UPDATE_STUDY_TIME",
      ...
      // 5. Simpan penanda tanggal hari ini
      await AsyncStorage.setItem(LAST_STUDY_DATE_KEY, todayStr);
      console.log("✅ [DAILY RESET] Sukses reset lokal & antrean sync!");
      return true; // Berhasil melakukan reset
    } catch (error) {
      console.error("❌ [DAILY RESET] Gagal melakukan reset:", error);
      return false;
    }
  }
