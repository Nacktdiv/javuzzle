...
function ProfileContent() {
  ...
  // Helper simpan ke sync queue bila offline
  const saveToSyncQueue = (action: string, payload: any) => {
    db.runSync(
  ...
  const handleSaveProfileUpdate = async (value1: string) => {
    ...
    try {
      ...
      db.runSync(
        `UPDATE users SET ${field} = ?, is_synced = ? WHERE id = ?`,
        [formattedValue, isOnline ? 1 : 0, user.id]
      );
      ...
      if (isOnline) {
        ...
        if (error) {
          saveToSyncQueue('UPDATE_USER_PROFILE', { userId: user.id, [field]: formattedValue });
      ...
      showAlert({ message: e.message || 'Gagal memperbarui data', title: 'Gagal' });
    }
  };
  ...
  const handleSavePasswordUpdate = async (pass: string, confirmPass?: string) => {
    ...
    try {
      ...
      if (!netState.isConnected) {
        showAlert({ message: 'Perubahan password membutuhkan koneksi internet!', title: 'Perhatian' });
        return;
      }
    ...
    showAlert({
      ...
      onConfirmPressed: async () => {
        try {
          ...
          console.log('Logout offline:', e);
        } finally {
          // Bersihkan data user lokal saat logout
          if (user?.id) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.helpBtn} onPress={handleStartWalkthrough} activeOpacity={0.7}>
